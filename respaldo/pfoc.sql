...
BEGIN
    ...
    SELECT
        usua_ID_comp_param,
        NOW(),
        SUM(pi.Precio), -- Utilizamos pi.Precio en lugar de producto.Precio
        tarjeta_id_var,
        cali_id_var,
        vent_estatus_param
    FROM
        Carrito carrito
    JOIN
        Producto_Info pi ON carrito.Prod_ID = pi.Prod_ID
    WHERE
    ...
    SELECT
        ...
        carrito.Cantidad,
        pi.Precio,
        carrito.Cantidad * pi.Precio, -- Utilizamos pi.Precio en lugar de producto.Precio
        1
    FROM
        Carrito carrito
    JOIN
