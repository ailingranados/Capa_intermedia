...
if (isset($_GET['validado'])) {
    ...
    switch ($validado) {
        ...
        JOIN 
            Usuario u ON pi.Usua_ID = u.Usua_ID where  pi.PrIn_Validado = 0 AND (
                p.Prod_Nombre LIKE '%$busqueda%'
                OR pi.PrIn_Descripcion LIKE '%$busqueda%'
                OR c.Cate_Nombre LIKE '%$busqueda%'
                OR u.Usua_Nombre LIKE '%$busqueda%'
            )";
            break;
        case 1:
            $sql = "SELECT 
        ...
        JOIN 
            Categorias c ON pi.Cate_ID = c.Cate_ID
        JOIN 
            Usuario u ON pi.Usua_ID = u.Usua_ID where  pi.PrIn_Validado = 1 AND (
                p.Prod_Nombre LIKE '%$busqueda%'
                OR pi.PrIn_Descripcion LIKE '%$busqueda%'
                OR c.Cate_Nombre LIKE '%$busqueda%'
                OR u.Usua_Nombre LIKE '%$busqueda%'
        ...
        JOIN 
            Categorias c ON pi.Cate_ID = c.Cate_ID
        JOIN 
            Usuario u ON pi.Usua_ID = u.Usua_ID where (
                p.Prod_Nombre LIKE '%$busqueda%'
                OR pi.PrIn_Descripcion LIKE '%$busqueda%'
                OR c.Cate_Nombre LIKE '%$busqueda%'
                OR u.Usua_Nombre LIKE '%$busqueda%'
    ...
    if ($result->num_rows > 0) {
        ...
        echo "
                    ...
                    <th> <a href='Producto.php?usuario=". $usuario ."&prod_id=".$producto_id."&usuarioid=". $idd ."'>". "Ver producto" . "</a> </th>
                </tr>";
        }
    } else {
